<div class="clearfix"></div>
<div class="row">
    <footer class="footer footer-a col-lg-12 ">
        <div class="col-12 top-footer">
            <div class="col-4">   
                <ul>        
                    <li class="menu"><p><a href="<?php echo base_url() ?>welcome/gotoCGU">CGU</a></p></li>
                </ul>
            </div>

            <div class="col-4">   
                <ul>        
                    <li class="menu"><p><a href="<?php echo base_url() ?>welcome/gotoLegal">Mentions Légales</a></p></li>


                </ul>
            </div>
            <div class=" col-4">   
                <ul>        
                    <li class="menu"><p><a href="mailto:realdate.aix@gmail.com">Envoyer un e-mail</a></p></li>                   
                </ul>
            </div>
            <div class="col-12 ">
                <div class="footer-medias">
                    <div class="social-link">
                        <a href="https://www.facebook.com/RealDate.fr/"> <img src="<?php echo base_url() ?>assets/images/rd-fb.png" alt="logo instagram"/> </a>
                        <a href="https://www.instagram.com/real_date13100/">  <img src="<?php echo base_url() ?>assets/images/rd-insta.png" alt="logo instagram"/></a>

                    </div>
                   
                </div>

            </div>
    </div>
</footer>
</div>
</body>
